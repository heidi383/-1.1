...
void picture() {
	...
	ears->setOnMouseCallback([&](ObjectPtr object, int x, int y, MouseAction action)->bool {
		...
		auto check1 = Object::create("image/체크.png", sceneGame, 100, 170);
	...
	arms->setOnMouseCallback([&](ObjectPtr object, int x, int y, MouseAction action)->bool {
		...
		auto check2 = Object::create("image/체크.png", sceneGame, 410, 120);
	...
	shoes->setOnMouseCallback([&](ObjectPtr object, int x, int y, MouseAction action)->bool {
		...
		auto check3 = Object::create("image/체크.png", sceneGame, 700, 100);
	...
	hat->setOnMouseCallback([&](ObjectPtr object, int x, int y, MouseAction action)->bool {
		...
		auto check4 = Object::create("image/체크.png", sceneGame, 240, 530);
	...
	eyes->setOnMouseCallback([&](ObjectPtr object, int x, int y, MouseAction action)->bool {
		...
		auto check5 = Object::create("image/체크.png", sceneGame, 330, 520);
	...
	mouth->setOnMouseCallback([&](ObjectPtr object, int x, int y, MouseAction action)->bool {
		...
		auto check6 = Object::create("image/체크.png", sceneGame, 600, 400);
		check6->setScale(0.6f);
		count = count + 1;
		if (count == 6) {
			showMessage("!성공!");
			sceneEnd->enter();
...
int main() {
	...
	auto next = Object::create("image/NEXT.png", sceneEnd, 450, 300);
	next->setScale(0.6f);
	...
	auto cbread = Object::create("image/c빵.png", scenePlay, 1100, 400);
	cbread->setScale(0.7f);
	auto ctoma = Object::create("image/c토마토.png", scenePlay, 1100, 350);
	ctoma->setScale(0.7f);
	auto cpa = Object::create("image/c패티.png", scenePlay, 1100, 300);
	cpa->setScale(0.7f);
	auto ccab = Object::create("image/c양상추.png", scenePlay, 1100, 250);
	ccab->setScale(0.7f);
	auto cche = Object::create("image/c치즈.png", scenePlay, 1100, 200);
	cche->setScale(0.7f);
	...
	timer->setOnTimerCallback([&](auto)->bool {
		...
		timer->start();
		if ((breadX - 70 < manX) && (manX < breadX + 50) && (50 < breadY) && (breadY < 150)) {
			cbread->hide();
			s1 = 1;
		}
		if ((tomaX - 70 < manX) && (manX < tomaX + 50) && (50 < tomaY) && (tomaY < 150)) {
			ctoma->hide();
			s2 = 1;
		}
		if ((paX - 70 < manX) && (manX < paX + 50) && (50 < paY) && (paY < 150)) {
			cpa->hide();
			s3 = 1;
		}
		if ((cabX - 70 < manX) && (manX < cabX + 50) && (50 < cabY) && (cabY < 150)) {
			ccab->hide();
			s4 = 1;
		}
		if ((cheX - 70 < manX) && (manX < cheX + 50) && (50 < cheY) && (cheY < 150)) {
			cche->hide();
			s5 = 1;
		}
